...
S21Matrix::S21Matrix(int rows, int columns) : _rows(rows), _columns(columns), _matrix(nullptr) {
    if (rows <= 0 || columns <= 0) {
        throw S21MatrixExcepcion("Incorrect matrix size. Rows and columns must be more, than 0");
    }
...
S21Matrix::S21Matrix(S21Matrix&& object) : _rows(0), _columns(0), _matrix(nullptr) {
    if (object._rows <= 0 || object._columns <= 0) {
        throw S21MatrixExcepcion("Incorrect matrix size. Rows and columns must be more, than 0");
    }
...
void S21Matrix::mul_matrix(const S21Matrix& object) {
    if (_columns != object._rows) {
        throw S21MatrixExcepcion("Number of columns first is not equal to number of rows second matrix");
    }
...
S21Matrix S21Matrix::multWithSigmoid(const S21Matrix& object) {
    if (_columns != object._rows) {
        throw S21MatrixExcepcion("Number of columns first is not equal to number of rows second matrix");
    }
    ...
    for (int i = 0; i < _rows; i++) {
        for (int j = 0; j < object._columns; j++) {
            for (int k = 0; k < object._rows; k++) {
                resultMatrix(i, j) += _matrix[i][k] * object._matrix[k][j];
            }

            resultMatrix(i, j) = 1 / (1 + exp(-resultMatrix(i, j)));
            // std::cout << resultMatrix(i, j) << std::endl;
        }
    }
...
double S21Matrix::determinant() {
    ...
    if (_rows == 1) {
        ...
        determinant = (_matrix[0][0] * _matrix[1][1]) - (_matrix[0][1] * _matrix[1][0]);
    } else {
        for (int i = 0; i < _rows; i++) {
...
bool S21Matrix::operator==(const S21Matrix& object) { return this->eq_matrix(object); }
...
void S21Matrix::fillMatrix(double** destMatrix, double** srcMatrix, int rows, int columns) {
...
void S21Matrix::fillMatrixRandomValue() {
    ...
    for (int i = 0; i < getRows(); i++) {
        for (int j = 0; j < getColumns(); j++) {
            this->operator()(i, j) = distr(eng);
        }
    }
}

void S21Matrix::fillMatrixFromVector(const std::vector<double> data) {
    int vectorCounter = 0;

    for (int i = 0; i < getRows(); i++) {
        for (int j = 0; j < getColumns(); j++) {
            this->operator()(i, j) = data.at(vectorCounter);
            vectorCounter++;
        }
    }
}
