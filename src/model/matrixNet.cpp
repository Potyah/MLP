...
void MatrixNet::setLearningRate(double learningRate) { m_LearningRate = learningRate; }
...
void MatrixNet::setLayersCount(int hidenLayersQuantity) { m_LayersCount = hidenLayersQuantity; }
...
void MatrixNet::calculateLastLayerError(const int expectedResult, const int layer) {
...
void MatrixNet::adjustWeight(int layer) {
    for (int i = 0; i < m_Output[layer].getColumns(); i++) {
        for (int j = 0; j < m_Output[layer - 1].getColumns(); j++) {
            ...
            m_Weights[layer](j, i) =
                m_Weights[layer](j, i) - prevNeuronOut * m_Delta[layer](0, i) * m_LearningRate;
        }
    }
}
