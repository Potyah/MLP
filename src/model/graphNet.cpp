...
void GraphNet::setLearningRate(double learningRate) {
...
void GraphNet::loadNet(std::string path) {
  ...
  while (!fs.eof()) {
    ...
    while (st >> neuronData) {
      m_Layers.at(layerNumber)
          ->getNeuron()
          .at(neuronCount)
          ->setWeight(count, neuronData);
      count++;
    }
    neuronCount++;
  }
...
bool GraphNet::checkTrainingResult(int expectedResult) {
  return (int)getLetter() - 64 == expectedResult;
}
