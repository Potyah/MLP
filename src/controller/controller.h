...
public:
    Controller();
    ~Controller();
    void setNetwork(ModelType modelType, int layersCount, std::string pathTest, std::string pathTrain);
    void deleteNetwork();
    void updateNetwork(ModelType modelType, int layersCount, std::string pathTest, std::string pathTrain);
    void updateNetwork(ModelType modelType, int layersCount);
    std::array<double, 6> testFile(double sampleSize);
    std::vector<double> trainingNetwork(unsigned long crossValidationElements, unsigned int epoch);
